...
import com.ziyun.academic.dao.A3HotspotDao;
import com.ziyun.academic.dao.MonitorDao;
import com.ziyun.academic.dao.OnLineNumDao;
import com.ziyun.academic.entity.A3Hotspot;
import com.ziyun.academic.entity.FireWallFlowMonitor;
...
public class MonitorServerImpl implements MonitorServer {
    ...
    @Autowired
    private A3HotspotDao a3HotspotDao;
    @Autowired
    private OnLineNumDao onLineNumDao;
    ...
    public int getOnLineNum() {
        return monitorDao.getOnLineNum();
    }
    ...
    public List<LinkedHashMap<String, Object>> getOnLinePersion() {
        return monitorDao.getOnLinePersion();
    }

    @Override
//	@Cacheable(value="baseCache",key="#root.target.FLOW_LIST_KEY")
    public Map<String, Object> getIpgeo() {
        Map<String, Object> result = new HashMap<String, Object>();
        // 查詢所有城市信息
        List<LinkedHashMap<String, String>> cityMaps = monitorDao.getCityList();
    ...
    public List<HotSpotVo> getTopHotSpot(Params para) {
        ...
        List<A3Hotspot> spots = a3HotspotDao.listToday(para);
        ...
        redisTemplate.opsForValue().set(timestamp, hotspotMap, 43200);
        ...
        for (Map.Entry<String, Object> entry : hotspotMap.entrySet()) {
            ...
            vo.setType(key);
            vo.setNum(Integer.parseInt(String.valueOf(entry.getValue())));
            volist.add(vo);
        }
    ...
    public List<Map> getOnLineNumHis() {
        List<Map> onLineNumList = onLineNumDao.listOnLineNum();
        if (null == onLineNumList || onLineNumList.size() == 0) {
            return null;
        }
        //获取当前在线人数，判断是否大于0，大于0 的时候存入
        int nowStudent = monitorDao.getOnLineNum();
        if (nowStudent > 0) {
    ...
    public Map getOnlineStudent(ParamsStatus param) {
        ...
        if (param.getTimeStamp() != null) {
            ...
            if (redisTemplate.hasKey(param.getTimeStamp())) {
                ...
                lists = monitorDao.getOnlineStudent();
    ...
    public Map<String, Object> getWorldIp() {
        List<LinkedHashMap<String, String>> cityLists = monitorDao.getCityList();
