...
public class RedisUtils {
     ...
     * 默认过期时间
     */
    private final int expireTime = Constant.LOGIN_EXPIRE_TIME;
    ...
    public void expireUserCache(String username, Long roleId) {
        redisTemplate.expire(CacheConstant.USER_CACHE + CacheConstant.DELIMITER_CACHE + username + CacheConstant.DELIMITER_CACHE + roleId + CacheConstant.SALT, expireTime, TimeUnit.SECONDS);
    }
    ...
    public void expireUsernameCache(String username, Long roleId) {
        redisTemplate.expire(CacheConstant.USERNAME_CACHE + CacheConstant.DELIMITER_CACHE + username + CacheConstant.DELIMITER_CACHE + roleId + CacheConstant.SALT, expireTime, TimeUnit.SECONDS);
    }
     ...
     *
     * @param username     用户名
     * @param access_token oauth 生成的token
     */
    public void setAccessTokenCache(String username, Long roleId, String access_token, Integer expireTime) {
        if (expireTime == null || expireTime == 0) {
            //使用默认过期时间
            expireTime = this.expireTime;
        }
        redisTemplate.opsForValue().set(CacheConstant.ACCESSTOKEN_CACHE + CacheConstant.DELIMITER_CACHE + username + CacheConstant.DELIMITER_CACHE + roleId + CacheConstant.SALT, access_token, expireTime, TimeUnit.SECONDS);
    }
    ...
    public void expireAccessTokenCache(String username, Long roleId) {
        redisTemplate.expire(CacheConstant.ACCESSTOKEN_CACHE + CacheConstant.DELIMITER_CACHE + username + CacheConstant.DELIMITER_CACHE + roleId + CacheConstant.SALT, expireTime, TimeUnit.SECONDS);
    }
     ...
     *
     * @param username      用户名
     * @param refresh_token oauth2返回的刷新token
     * @param expireTime    过期时间
     */
    public void setRefreshTokenCache(String username, Long roleId, String refresh_token, Integer expireTime) {
        if (expireTime == null || expireTime == 0) {
            //refresh_token 默认设置一周
            expireTime = Constant.REFRESH_TOKEN_EXPIRE_TIME;
        }
        redisTemplate.opsForValue().set(CacheConstant.REFRESHTOKEN_CACHE + CacheConstant.DELIMITER_CACHE + username + CacheConstant.DELIMITER_CACHE + roleId + CacheConstant.SALT, refresh_token, expireTime, TimeUnit.SECONDS);
    }
